...
class TestHandleChat:
    ...
    def test_chat_cwd_is_koan_root_not_project_path(
        ...
        cwd_used = mock_run_cli.call_args_list[0].kwargs.get("cwd")
        assert cwd_used == str(koan_root), (
            f"Chat cwd should be KOAN_ROOT ({koan_root}), "
            f"not PROJECT_PATH ({project_path}). Got: {cwd_used}"
        )
